...
(function(E) {
	...
	(function(a) {
		var q = a.deg2rad,
			k = a.pick;
		a.perspective = function(p, n, u) {
			var m = n.options.chart.options3d,
				g = u ? n.inverted : !1,
				h = n.plotWidth / 2,
				r = n.plotHeight / 2,
				e = m.depth / 2,
				f = k(m.depth, 1) * k(m.viewDistance, 0),
				d = n.scale3d || 1,
				b = q * m.beta * (g ? -1 : 1),
				m = q * m.alpha * (g ? -1 : 1),
				c = Math.cos(m),
			...
			return a.map(p, function(b) {
				...
				b = -x * y * m + c * n - w * x * k;
				m = c * y * m + x * n + c * w * k;
				n = 0 < f && f < Number.POSITIVE_INFINITY ? f / (m + e + f) : 1;
				a = a * n * d + h;
				b = b * n * d + r;
				return {
					x: g ? b : a,
		...
		a.pointCameraDistance = function(a, n) {
			var p = n.options.chart.options3d,
				m = n.plotWidth / 2;
			n = n.plotHeight / 2;
			p = k(p.depth, 1) * k(p.viewDistance, 0) + p.depth;
			return Math.sqrt(Math.pow(m - a.plotX, 2) + Math.pow(n - a.plotY, 2) + Math.pow(p - a.plotZ, 2))
		}
	})(E);
	(function(a) {
		function q(b) {
			var d = 0,
				l, C;
			for(l = 0; l < b.length; l++) C = (l + 1) % b.length, d += b[l].x * b[C].y -
				b[C].x * b[l].y;
			return d / 2
		}
		...
		function k(b) {
			var d = 0,
				l;
			for(l = 0; l < b.length; l++) d += b[l].z;
			return b.length ? d / b.length : 0
		}

		function p(b, d, l, C, a, c, e, f) {
			var g = [],
				H = c - a;
			return c > a && c - a > Math.PI / 2 + .0001 ? (g = g.concat(p(b, d, l, C, a, a + Math.PI / 2, e, f)), g = g.concat(p(b, d, l, C, a + Math.PI / 2, c, e, f))) : c < a && a - c > Math.PI / 2 + .0001 ? (g = g.concat(p(b, d, l, C, a, a - Math.PI / 2, e, f)), g = g.concat(p(b, d, l, C, a - Math.PI / 2, c, e, f))) : ["C", b + l * Math.cos(a) - l * t * H * Math.sin(a) + e, d + C * Math.sin(a) + C * t * H * Math.cos(a) + f, b + l * Math.cos(c) + l * t * H * Math.sin(c) + e, d + C * Math.sin(c) -
		...
		var n = Math.cos,
			...
			G = a.SVGRenderer,
			B = a.wrap,
			t = 4 * (Math.sqrt(2) - 1) / 3 / (u / 2);
		G.prototype.toLinePath = function(b, a) {
			var c = [];
			d(b, function(b) {
				c.push("L", b.x, b.y)
			});
			b.length && (c[0] = "M", a && c.push("Z"));
			return c
		};
		G.prototype.cuboid = function(b) {
			...
			d.top = this.path(b[1]).attr({
				"class": "highcharts-3d-top",
				zIndex: b[4]
			}).add(d);
			d.side = this.path(b[2]).attr({
			...
			d.attr = function(b, d) {
				...
				if(b.shapeArgs || e(b.x)) b = this.renderer.cuboidPath(b.shapeArgs || b), this.front.attr({
					d: b[0],
					zIndex: b[3]
				}), this.top.attr({
					d: b[1],
			...
			d.animate = function(b, d, c) {
				e(b.x) && e(b.y) ? (b = this.renderer.cuboidPath(b), this.front.attr({
					zIndex: b[3]
				}).animate({
						d: b[0]
					},
					...
					zIndex: -b[6]
				})) : b.opacity ? (this.front.animate(b, d, c), this.top.animate(b, d, c), this.side.animate(b, d, c)) : A.prototype.animate.call(this, b, d, c);
				return this
			};
			d.destroy = function() {
				this.front.destroy();
		...
		G.prototype.cuboidPath = function(b) {
			...
			var c = b.x,
				...
				m = y(m, h[this.chartIndex], b.insidePlotArea),
				e = function(b, c) {
					var a = [];
					b = w(b, d);
					c = w(c, d);
			...
			a = e([1, 6, 7, 0], c);
			e = e([1, 2, 5, 6], [0, 7, 4, 3]);
			return [this.toLinePath(b, !0), this.toLinePath(a, !0), this.toLinePath(e, !0), k(b), k(a), k(e), 9E9 * k(w(c, d))]
		};
		a.SVGRenderer.prototype.arc3d = function(a) {
			function e(b) {
				var d = !1,
					a = {};
				b = x(b);
				for(var e in b) - 1 !== c(e, n) && (a[e] = b[e], delete b[e], d = !0);
				return d ? a : !1
			}
			var l = this.g(),
				m = l.renderer,
				n = "x y r innerR start end".split(" ");
			a = x(a);
			a.alpha *= f;
			a.beta *= f;
			l.top = m.path();
			...
			l.onAdd = function() {
				var b = l.parentGroup,
					a = l.attr("class");
				l.top.add(l);
				d(["out", "inn", "side1", "side2"], function(d) {
					l[d].addClass(a + " highcharts-3d-side").add(b)
				})
			};
			l.setPaths = function(b) {
				var d = l.renderer.arc3dPath(b),
					a = 100 * d.zTop;
				l.attribs = b;
				l.top.attr({
					d: d.top,
			...
			d(["opacity", "translateX",
				"translateY", "visibility"
			], function(b) {
				l[b + "Setter"] = function(b, a) {
					l[a] = b;
					d(["out", "inn", "side1", "side2", "top"], function(d) {
						l[d].attr(a, b)
					})
				}
			});
			B(l, "attr", function(d, a) {
				var c;
				"object" === typeof a && (c = e(a)) && (b(l.attribs, c), l.setPaths(l.attribs));
				return d.apply(this, [].slice.call(arguments, 1))
			});
			B(l, "animate", function(b, d, a, c) {
				var l, f = this.attribs,
					m;
				delete d.center;
				delete d.z;
				delete d.depth;
				delete d.alpha;
				delete d.beta;
				m = g(F(a, this.renderer.globalAnimation));
				m.duration && (l = e(d), d.dummy = 1, l && (m.step =
					function(b, d) {
						function a(b) {
							return f[b] + (F(l[b], f[b]) - f[b]) * d.pos
						}
						"dummy" === d.prop && d.elem.setPaths(x(f, {
							x: a("x"),
							y: a("y"),
							r: a("r"),
							innerR: a("innerR"),
		...
		G.prototype.arc3dPath = function(b) {
			...
			var y = Math.sin(f),
				...
				z = w * Math.sin(h),
				w = ["M", a + v * x, c + g * q],
				w = w.concat(p(a, c, v, g, e, f, 0, 0)),
				w = w.concat(["L", a + t * b, c + B * y]),
				w =
				w.concat(p(a, c, t, B, f, e, 0, 0)),
				w = w.concat(["Z"]),
				F = 0 < k ? Math.PI / 2 : 0,
				k = 0 < h ? 0 : Math.PI / 2,
				F = e > -F ? e : f > -F ? -F : e,
				D = f < u - k ? f : e < u - k ? u - k : f,
				A = 2 * u - k,
				h = ["M", a + v * n(F), c + g * m(F)],
				h = h.concat(p(a, c, v, g, F, D, 0, 0));
			f > A && e < A ? (h = h.concat(["L", a + v * n(D) + r, c + g * m(D) + z]), h = h.concat(p(a, c, v, g, D, A, r, z)), h = h.concat(["L", a + v * n(A), c + g * m(A)]), h = h.concat(p(a, c, v, g, A, f, 0, 0)), h = h.concat(["L", a + v * n(f) + r, c + g * m(f) + z]), h = h.concat(p(a, c, v, g, f, A, r, z)), h = h.concat(["L", a + v * n(A), c + g * m(A)]), h = h.concat(p(a, c, v, g, A, D, 0, 0))) : f > u - k && e < u - k && (h = h.concat(["L",
				a + v * Math.cos(D) + r, c + g * Math.sin(D) + z
			]), h = h.concat(p(a, c, v, g, D, f, r, z)), h = h.concat(["L", a + v * Math.cos(f), c + g * Math.sin(f)]), h = h.concat(p(a, c, v, g, f, D, 0, 0)));
			h = h.concat(["L", a + v * Math.cos(D) + r, c + g * Math.sin(D) + z]);
			h = h.concat(p(a, c, v, g, D, F, r, z));
			h = h.concat(["Z"]);
			k = ["M", a + t * x, c + B * q];
			k = k.concat(p(a, c, t, B, e, f, 0, 0));
			k = k.concat(["L", a + t * Math.cos(f) + r, c + B * Math.sin(f) + z]);
			k = k.concat(p(a, c, t, B, f, e, r, z));
			k = k.concat(["Z"]);
			x = ["M", a + v * x, c + g * q, "L", a + v * x + r, c + g * q + z, "L", a + t * x + r, c + B * q + z, "L", a + t * x, c + B * q, "Z"];
			a = ["M", a + v * b,
				c + g * y, "L", a + v * b + r, c + g * y + z, "L", a + t * b + r, c + B * y + z, "L", a + t * b, c + B * y, "Z"
			];
			y = Math.atan2(z, -r);
			c = Math.abs(f + y);
			b = Math.abs(e + y);
	...
	(function(a) {
		function q(a, e) {
			...
			f > r && (t = Math.min(t, 1 - Math.abs((f + g) / (r + g)) % 1));
			d < m && (t = Math.min(t, (d - g) / (m - g)));
			b > k && (t = 0 > k ? Math.min(t, (b + h) / (-k + b + h)) : Math.min(t, 1 - (b + h) / (k + h) % 1));
			c < n && (t = Math.min(t, Math.abs((c - h) / (n - h))));
			return t
		}
		var k = a.Chart,
			p = a.each,
		...
		a.wrap(a.Chart.prototype, "setChartSize", function(a) {
			var e = this.options.chart.options3d;
			a.apply(this, [].slice.call(arguments, 1));
			if(this.is3d()) {
				var f = this.inverted,
					d = this.clipBox,
				...
				d[f ? "y" : "x"] = -(b[3] || 0);
				d[f ? "x" : "y"] = -(b[0] || 0);
				d[f ? "height" : "width"] = this.chartWidth + (b[3] || 0) + (b[1] || 0);
				d[f ? "width" : "height"] =
					this.chartHeight + (b[0] || 0) + (b[2] || 0);
				this.scale3d = 1;
				!0 === e.fitToPlot && (this.scale3d = q(this, e.depth))
			}
		});
		...
		g(k.prototype, "renderSeries", function(a) {
			...
			if(this.is3d())
				for(; e--;) a = this.series[e], a.translate(), a.render();
			else a.call(this)
		});
	...
	(function(a) {
		...
		f(k.prototype, "render", function(a) {
			...
			if(this.chart.is3d() && "colorAxis" !==
				...
				this.isZAxis || (this.horiz ? (g = {
					x: r,
					y: n + (b.xAxis[0].opposite ? -f.size : k),
					z: 0,
					width: m,
					height: f.size,
					depth: h,
					...
					zIndex: b.yAxis[0].reversed && 0 < d.alpha ? 4 : -1
				}).add(), this.bottomFrame.attr({
					fill: f.color ||
						"none",
					stroke: f.color || "none"
				}))) : (d = {
					x: r + (b.yAxis[0].opposite ? 0 : -e.size),
					y: n + (b.xAxis[0].opposite ? -f.size : 0),
					z: h,
					width: m + e.size,
					height: k + f.size,
					depth: g.size,
					...
					x: r + (b.yAxis[0].opposite ? m : -e.size),
					y: n + (b.xAxis[0].opposite ? -f.size : 0),
					z: 0,
					width: e.size,
					height: k + f.size,
		...
		f(k.prototype, "getPlotLinePath", function(a) {
			...
			var c = this.chart,
				...
				c = this.isZAxis ? c.plotWidth : d.depth,
				d = this.opposite;
			this.horiz && (d = !d);
			b = [this.swapZ({
				x: b[1],
		...
		f(k.prototype, "getLinePath", function(a) {
			return this.chart.is3d() ? [] : a.apply(this, [].slice.call(arguments, 1))
		});
		f(k.prototype, "getPlotBandPath", function(a) {
		...
		f(e.prototype, "getMarkPath", function(a) {
			var b = a.apply(this, [].slice.call(arguments, 1));
		...
		f(e.prototype, "getLabelPosition", function(a) {
			var b = a.apply(this, [].slice.call(arguments, 1));
		...
		a.wrap(k.prototype, "getTitlePosition", function(a) {
			var b = this.chart.is3d() && "colorAxis" !== this.coll,
				c, d;
		...
		f(k.prototype, "drawCrosshair", function(a) {
			var b = arguments;
		...
		f(k.prototype, "destroy", function(a) {
			n(["backFrame", "bottomFrame", "sideFrame"], function(b) {
		...
		f(p.prototype, "getAxes", function(a) {
			var b = this,
				c = this.options,
				c = c.zAxis = r(c.zAxis || {});
			a.call(this);
			b.is3d() && (this.zAxis = [], n(c, function(a, c) {
				a.index = c;
				a.isX = !0;
				(new q(b, a)).setScale()
			}))
	...
	(function(a) {
		function q(a) {
			...
			this.chart.is3d() && (e.stroke = this.options.edgeColor || e.fill, e["stroke-width"] = u(this.options.edgeWidth, 1));
			return e
		}
		...
		a(g.column.prototype, "translate", function(a) {
			...
			if(this.chart.is3d()) {
				var e = this.chart,
					f = this.options,
					d = f.depth || 25,
					b = (f.stacking ? f.stack || 0 : this._i) * (d + (f.groupZPadding || 1));
				!1 !== f.grouping && (b = 0);
				b += f.groupZPadding || 1;
		...
		a(g.column.prototype, "animate",
			function(a) {
				if(this.chart.is3d()) {
					...
					h && (e ? p(d.data, function(a) {
						null !== a.y && (a.height = a.shapeArgs.height, a.shapey = a.shapeArgs.y, a.shapeArgs.height = 1, b || (a.shapeArgs.y = a.stackY ? a.plotY + f.translate(a.stackY) : a.plotY + (a.negative ? -a.height : a.height)))
					}) : (p(d.data, function(a) {
						null !== a.y && (a.shapeArgs.height = a.height, a.shapeArgs.y = a.shapey, a.graphic && a.graphic.animate(a.shapeArgs, d.options.animation))
					}), this.drawDataLabels(), d.animate = null))
		...
		a(g.column.prototype, "init", function(a) {
			...
			if(this.chart.is3d()) {
				var e = this.options,
					f = e.grouping,
					d = e.stacking,
					b = u(this.yAxis.options.reversedStacks, !0),
					c = 0;
				if(void 0 === f || f) {
					f = this.chart.retrieveStacks(d);
					c = e.stack || 0;
					for(d = 0; d < f[c].series.length && f[c].series[d] !== this; d++);
					c = 10 * (f.totalStacks - f[c].position) + (b ? d : -d);
					this.xAxis.reversed || (c = 10 * f.totalStacks - c)
				}
				e.zIndex = c
			}
		});
		a(g.column.prototype, "pointAttribs", q);
		g.columnrange && a(g.columnrange.prototype,
			"pointAttribs", q);
		a(m.prototype, "alignDataLabel", function(a) {
	...
	(function(a) {
		var q = a.deg2rad,
			...
			u = a.svg;
		a = a.wrap;
		a(n.pie.prototype, "translate", function(a) {
			...
			if(this.chart.is3d()) {
				var g = this,
					h = g.options,
					m = h.depth || 0,
					e = g.chart.options.chart.options3d,
					f = e.alpha,
					d = e.beta,
					b = h.stacking ? (h.stack || 0) * m : g._i * m,
					b = b + m / 2;
				!1 !== h.grouping && (b = 0);
				k(g.data, function(a) {
					var c = a.shapeArgs;
					...
					c = (c.end + c.start) / 2;
					a.slicedTranslation = {
						translateX: Math.round(Math.cos(c) * h.slicedOffset * Math.cos(f * q)),
						translateY: Math.round(Math.sin(c) * h.slicedOffset * Math.cos(f * q))
					}
				})
			}
		});
		...
		a(n.pie.prototype, "drawDataLabels",
			function(a) {
				if(this.chart.is3d()) {
					...
					k(this.data, function(a) {
						var h = a.shapeArgs,
							...
							f = (h.start + h.end) / 2,
							d = a.labelPos,
							b = -e * (1 - Math.cos((h.alpha || g.alpha) * q)) * Math.sin(f),
							c = e * (Math.cos((h.beta || g.beta) * q) - 1) * Math.cos(f);
						k([0, 2, 4], function(a) {
							d[a] += c;
							d[a + 1] += b
						})
		...
		a(n.pie.prototype, "addPoint", function(a) {
			a.apply(this, [].slice.call(arguments, 1));
			this.chart.is3d() && this.update(this.userOptions, !0)
		});
		a(n.pie.prototype, "animate",
			function(a) {
				if(this.chart.is3d()) {
					var g = arguments[1],
						...
						e = this.group,
						f = this.markerGroup;
					u && (!0 === h && (h = {}), g ? (e.oldtranslateX = e.translateX, e.oldtranslateY = e.translateY, g = {
						translateX: k[0],
						translateY: k[1],
						scaleX: .001,
						scaleY: .001
					}, e.attr(g), f && (f.attrSetters = e.attrSetters, f.attr(g))) : (g = {
						translateX: e.oldtranslateX,
						translateY: e.oldtranslateY,
						scaleX: 1,
						scaleY: 1
					}, e.animate(g, h), f && f.animate(g, h), this.animate = null))
				} else a.apply(this, [].slice.call(arguments, 1))
			})
	})(E);
	(function(a) {
		var q = a.perspective,
			...
			n = a.seriesTypes,
			u = a.wrap;
		u(n.scatter.prototype, "translate", function(a) {
			a.apply(this, [].slice.call(arguments, 1));
			if(this.chart.is3d()) {
				var g = this.chart,
					h = k(this.zAxis, g.options.zAxis[0]),
					m = [],
					e, f, d;
				for(d = 0; d < this.data.length; d++) e = this.data[d], f = h.isLog && h.val2lin ? h.val2lin(e.z) : e.z, e.plotZ = h.translate(f), e.isInside = e.isInside ? f >= h.min && f <= h.max : !1, m.push({
					x: e.plotX,
					y: e.plotY,
					z: e.plotZ
				});
				g = q(m, g, !0);
				for(d = 0; d < this.data.length; d++) e = this.data[d], h =
		...
		u(n.scatter.prototype, "pointAttribs", function(k, g) {
	...
	(function(a) {
		...
		p && (a.setOptions({
			animate: !1
		}), p.prototype.cuboid = k.prototype.cuboid, p.prototype.cuboidPath = k.prototype.cuboidPath, p.prototype.toLinePath = k.prototype.toLinePath, p.prototype.createElement3D = k.prototype.createElement3D, p.prototype.arc3d = function(a) {
			a = k.prototype.arc3d.call(this, a);
			a.css({
				zIndex: a.zIndex
			});
			return a
		}, a.VMLRenderer.prototype.arc3dPath = a.SVGRenderer.prototype.arc3dPath, a.wrap(q.prototype, "render", function(a) {
			a.apply(this, [].slice.call(arguments, 1));
